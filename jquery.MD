...
## Ne čuvajte stanje aplikacije u HTML-u

HTML služi za prikaz podataka, a ne za njihovo skladištenje. Često developeri upisuju podatke u `data` atribute, u `innerHTML`, u skrivena polja i slično. Stanja aplikacije treba čuvati u Javaskriptu, a kada korisnik inicira njihovu promenu, treba ih ažurirati u JS-u, i zatim ponovo renderovati HTML. Nikako obratno, jer to je zapravo anti-obrazac! (ažurirati HTML, pa pomoću JS-a saznavati stanje aplikacije). HTML je samo *view* i nikako ne treba da služi kao *model* ili *kontroler* (vidi [MVC](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller)).
