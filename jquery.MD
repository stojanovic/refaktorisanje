...
## Sadržaj
  ...
  * [Uklonite sav JavaScript iz HTML-a](#uklonite-sav-javascript-iz-html-a)
  * [Podelite kod u zasebne fajlove](#podelite-kod-u-zasebne-fajlove)
  * [Izolujte opseg skripti](#izolujte-opseg-skripti)
...
## Izolujte opseg skripti

Trenutno u fajlu imamo puno globalnih varijabli prilično uopštenih naziva, što žestoko zagađuje globalni prostor. Ukoliko varijabla istog naziva već postoji u drugom fajlu, ili biblioteci koju uključujemo, doćiće do kolizije i pucanja programa.
...
Zagrade oko funkcije znače da je ona anonimna, a zagrade nakon funkcije da se odmah poziva. Na ovaj način, sve što je ostalo unutra više nije dostupno za spoljni svet i ovu funkciju više niko ne može pozvati. Ukoliko naš modul ima neku spoljašnju zavisnost (u ovom slučaju jQuery), onda to eksplicirajte prosleđivanjem varijable:

```javascript
(function($) {
  // jQuery varijabla je unutra dostupan pod alijasom $

})(jQuery);
```

Našem modulu prosleđujemo globalnu `jQuery` varijablu, koju unutra koristimo pod alijasom `$`. Globalna varijabla će naravno biti dostupna i ako je ne prosledimo, ali ovako ekspliciramo zavisnost u slučaju da modul promeni okruženje.

<img src='https://lostechies.com/derekgreer/files/2012/02/closure3.png' width='400'>

Međutim, ako nam je nešto ipak potrebno iz zatvorenog modula, moramo to izvesti van. To ćemo uraditi tako što ćemo uskladištiti rezultat poziva anonimne funkcije u varijablu, koja postaje naš prvi Javascript modul:
...
Dve javne funkcije koje smo izvezli sada su dostupne u `mathModule` imenskom prostoru i možemo ih koristiti na sledeći način: `mathModule.toDegrees(45)`. Treća funkcija `toCelsius()`, koju nismo izvezli, ostaje privatna.
