# Kako organizovati jQuery?
## Vodič za refaktorisanje Javaskript koda

***Ovaj vodić je namenjen organizovanju koda na većim Javaskcript projekatima, koji ne koriste ni jedan front-end framework. Ukoliko koriste neki JS framework, treba pratiti njegove smernice.***
...
## Sadržaj
  ...
  * [Izdvojte varijable u posebnu sekciju](#izdvojte-varijable-u-posebnu-sekciju)
  * [Izdvojte event listenere u posebnu sekciju](#izdvojte-event-listenere-u-posebnu-sekciju)
  * [Izdvojte funkcije u posebnu sekciju](#izdvojte-funkcije-u-posebnu-sekciju)
...
Za početak, komentarima naznačite glavne odeljke koje ćemo imati na stranici. Vaša organizacija koda se može razlikovati, ali korisno je napraviti mini sadržaj na početku kao putokaz:
...
## Izdvojte varijable u posebnu sekciju
...
## Izdvojte event listenere u posebnu sekciju

U jQuery-u, `document ready` je glavni slušalac događaja, koji se okida tek kada je HTML učitan (bez slika i ostalog što se učitava naknadno). On nam jemči da će u trenutku izvršenja koda biti spremni svi HTML elementi.

Niži slušači događaja obično idu unutar njega. Ako ih ostavimo izvan rizikujemo da element koji ciljamo još nije spreman. Listeneri koji se kače na globalne objekte (`document` ili `window`) mogu ići zasebno, jer su nezavisni:
...
Postoji i drugo rešenje kako da vežemo okidač za element koji još uvek ne postoji. Ako, na primer, hoćemo da vežemo klik za klasu `.should-hide`, koja trenutno nije u HTML-u, čak ni nakon što klasa bude dodata u DOM, klik neće raditi:
...
Međutim, ako vežemo klik za globalni `document` objekat (koji je dostupan sve vreme), kad on bude kliknut, delegiraće klik potčinjenoj klasi ukoliko se ona u međuvremenu pojavi:
...
Između ova dva načina organizacije, ja preporučujem drugo rešenje jer time izbegavamo stepeničastu strukturu koda:
...
Ukoliko celu logiku trpamo u `document ready`, javascript fajl će nam veoma brzo postati jedno veliko *jQuery crevo*.

## Izdvojte funkcije u posebnu sekciju

Pretražite fajl i organizujte sve funkcije u posebnu sekciju, koja se nalazi na dnu. Obavezno koristiti deklaracije funkcija, a ne funkcionalne izraze, jer je [razlika](http://stackoverflow.com/questions/1013385/what-is-the-difference-between-a-function-expression-vs-declaration-in-javascrip) presudna (u drugom slučaju će pući aplikacija). Anonimne funkcije za sada ostavite gde jesu, ali neke od njih ćemo kasnije [imenovati](#imenujte-anonimne-funkcije). Sada naš odeljak sa funkcijama izgleda ovako:
...
Imenovanje anonimnih funkcija je važan korak kako bismo izbegli zloglasni [*callback pakao*](http://callbackhell.com/). Evo jednog primera zbrkanog koda (sa callbackhell.com), kojim se iz browsera šalje http zahtev serveru:
...
U gornjem kodu postoje dve anonimne funkcije. Hajde da ih imenujemo:
...
Gornja skripta za slanje formulara ima jedan anonimni objekat, koji nije toliko upadljiv:
...
U ovih nekoliko linija se zapravo mnogo toga dešava. Imamo event listener, parametarski objekat, jQuery selektor i izvršnu funkciju. Hajde da podešavanja slajdera pomerimo među ostale konfiguracije na "kontrolnoj tabli":
...
Nakon toga smeštamo klik okidač koji poziva funkciju u JS fajl, među ostale event listenere:
...
Glavni JS fajl je sada bolje organizovan, ali i dalje preveliki. Podelite ga u manje fajlove, nazvane po logičnim celinama za čiju funkcionalnost su nadležni (npr. `contact.js`, `header.js`, `slider.js`). U glavnom fajlu ostavite samo zajedničku funkcionalnost. Na primer, ako se slajder koristi samo na naslovnoj strani, premestite ga tamo. Ako se funkcija za slanje formulara koristi samo na kontakt stranici, smestite je tamo. Obavezno testirajte program posle svakog premeštanja.

Podela JS koda će nam olakšati razvoj i smanjiti vreme učitavanja, jer ćemo skripte specifične za neku stranicu učitavati samo ako je potrebno. Zajedno sa drugim koracima, ovo će nam pomoći da pređemo na modularnu organizaciju projekta.
...
Ceo fajl ćemo lako izolovati tako što ćemo ga zatvoriti u [closure](https://developer.mozilla.org/en/docs/Web/JavaScript/Closures), odnosno u [samoizvršujuću anonimnu funkciju](https://en.wikipedia.org/wiki/Immediately-invoked_function_expression):
...
U Javascriptu, [import](https://developer.mozilla.org/en/docs/web/javascript/reference/statements/import) izraz još uvek nije prirodno podržan, ali postoje razni učitavači modula. [AMD](https://en.wikipedia.org/wiki/Asynchronous_module_definition) alatke nam omogućuju da eksplicitno navedemo od kojih biblioteka naša skripta zavisi, pa se kod izvršava tek nakon što se zavisnosti učitaju.

## Organizujte varijable u objekte

Ukoliko u konfiguracionom delu fajla imate mnoštvo letećih varijabli, pokušajte da ih struktuirate, odnosno povežete u objekte. Na primer, ako imate:
...
Povezite sve srodne leteće varijable u jedan objekat:
```javascript
var robot = {
  height: 200,
  width: 300,
...
Osim varijabli, često u fajlu postoje i funkcije povezane sa objektom, na primer:

```javascript
var changeRobotImage = function() { /* do it */ }
var changeRobotSpeed = function() { /* do it */ }
```

Moguće je i njih pridružiti odgovarajućem objektu, pa tako dobijamo sve na jednom mestu:
```javascript
var robot = {
  height: 200,
  width: 1024,
  speed: 20,
  changeImage: function() { /* do it */ },
  changeSpeed: function() { /* do it */ }
}
```

Ovako organizovan objekat sada možemo premestiti u zaseban fajl, a veoma lako ga možemo refaktorisati u klasu.
