...
Obzirom da [import](https://developer.mozilla.org/en/docs/web/javascript/reference/statements/import) u Javaskriptu još uvek nije prirodno podržan, postoji gomila rešenja za učitavanje modula (npr. RequireJS, CommonJS, Webpack...), a možete sve to rešiti i ručno. [AMD](https://en.wikipedia.org/wiki/Asynchronous_module_definition) alatke nam omogućuju da eksplicitno navedemo od kojih modula zavisi naša skripta, pa se kod izvršava nakon što se zavisnosti učitaju (npr. ako je za skriptu neophodan jQuery, izvršiće se nakon što se jQuery učita).
